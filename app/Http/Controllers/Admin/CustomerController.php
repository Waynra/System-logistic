        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('status', function ($row) {
                    if ($row->customer_status == 'Belum Bayar') {
                        $status = '<span class="badge bg-dark">Belum Bayar</span>';
                    } else if ($row->customer_status == 'Bayar') {
                        $status = '<span class="badge bg-success">Bayar</span>';
                    } else {
                        $status = '-';
                    }
                ...
                ->addColumn('action', function ($row) {
                    $array = array(
                        ...
                        "customer_status" => trim(preg_replace('/[^A-Za-z0-9-]+/', '_', $row->customer_status)),
                    );
                    $button = '';
                ...
                ->rawColumns(['action', 'notelp', 'alamat', 'bayar', 'status'])->make(true);
        }
    }
